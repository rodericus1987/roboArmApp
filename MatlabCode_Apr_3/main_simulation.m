...
import java.net.*;
import java.io.*;
javaaddpath C:\Users\Rene\Documents\MASc\Term2\ECE1778\Project\code\include
...
o = O_HOME;
q = inverse_ax12(ax12, o, 3, config, ROLL_HOME);
...
if exist('connected')
    ...
    while(1)
        ...
        vibrate = 0;
        if (bytes_available >= 24) && mod(bytes_available,6) == 0
            ...
            if sumValue ~= 0 || gripperIn ~= currentGrip
                ...
                if tempSum == 0
                    ...
                    % accumulate the values in the data packet
                    for i = 1:(bytes_available/24)
                        rollIn = rollIn + 1000*value((i-1)*6+1);
                        pitchIn = pitchIn + 1000*value((i-1)*6+2);
                    end
                    disp('Wrist Mode')
                end
...
%             if norm(q - angles) < 0.1 4&& abs(gripCounts - getGripper(m)) < 20 && moveFlag
%                 output_stream   = connected.getOutputStream;
%                 d_output_stream = DataOutputStream(output_stream);
